...
import ServiceMode from '../service-mode-al3.js';
...
export async function refreshModalWindow() {
  ...
  const sqlData = await runSqlSelect('al3_select_all_elements');
...
console.log(settingsArray);
  ...
  for (const setting of settingsArray) {
    if (setting.attributeTag) {
      const sql = await runSqlSelect('al3_select_tag_from_attrs', { tag_name: setting.tag.replace('state', 'comand') });
      if (sql && sql.length) {

        const tagValue = parseFloat(sql[0].tag_value); // pārveido stringu uz decimālskaitli
        const value = isNaN(tagValue) ? null : tagValue.toFixed(1).toString(); // noapaļo skaitli līdz 1 ciparam aiz komata un pārvērš atpakaļ uz stringu. Ja vērtība nav skaitlis, uzstāda null
        setting.value = setting.tag === 'comand_ID203_FRxx' ? Math.floor(tagValue / 10).toString() : value; // ja tag ir 'comand_ID203_FRxx', tad iedalot ar 10, noapaļojot uz leju un pārveidojot atpakaļ uz stringu, pretējā gadījumā lieto 'value'


        //setting.value = setting.tag === 'comand_ID203_FRxx' ? Math.floor(sql[0].tag_value / 10) : sql[0].tag_value; 
        console.log(setting.value);
        console.log(typeof(sql[0].tag_value));
        // Jālabo lai nav cieti definēts
      } else {
        setting.value = null;
      }
    ...
    input.addEventListener('change', function ({ target }) {
      ...
      try {
        if (name === 'comand_ID100_FR') {
          ...
          runSqlUpdate('al3_update_service_interval', { el_id: id, interval: value });
        } else {
          setTag(name, value);
          console.log(name);
          console.log(value);
          ...
          try {
            if (developerMode) {
              ...
              if (value === 'maintenance') {
                await runSqlUpdate('al3_update_service_time', { el_id: elId });
              } else {
